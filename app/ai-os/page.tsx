...
export const metadata: Metadata = {
  ...
  description:
    "Build an AI system that knows your world, reaches the right information, performs repeatable work, and runs consistently.",
};

export default function AiOsPage() {
  const guide = getResource("the-ai-operating-system-playbook");
  if (!guide) notFound();
    ...
    <div className="relative min-h-full flex flex-col">
      ...
      <header className="border-b border-warm-border">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="font-serif text-base font-medium text-warm-text hover:text-warm-accent transition-colors">
            Alex Sidhu
          </Link>
          <Link href="/resources" className="text-sm text-warm-muted hover:text-warm-text transition-colors">
            Resources
          </Link>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 py-16 md:py-24">
        <div className="rounded-2xl border border-warm-border/70 bg-cream/70 backdrop-blur-md shadow-[0_10px_40px_-12px_rgba(28,25,23,0.13)] p-8 md:p-12">
          <div className="mb-10">
            <p className="text-xs tracking-[0.2em] uppercase text-warm-accent mb-6">Free playbook</p>
            ...
            <p className="text-lg text-warm-muted leading-relaxed">
              Your AI should do more than remember a few facts about you. This
              playbook shows you how to give it the right context, connect it to
              your work, teach it repeatable jobs, and build routines that keep
              running. Drop your email and it unlocks here.
            </p>
          </div>

          <hr className="border-warm-border mb-10" />
          <GuideCapture content={guide.content} highlights={guide.highlights} source="ai-os-landing" />
        </div>
      </main>
      ...
      <footer className="border-t border-warm-border">
        <div className="max-w-5xl mx-auto px-6 py-8 flex items-center justify-between">
          ...
          <Link href="/" className="text-sm text-warm-muted hover:text-warm-text transition-colors">
            alexsidhu.com
          </Link>
        </div>
      </footer>
